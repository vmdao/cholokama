...
// view workspace
    ...
    globals.initialize = function (data) {
        ...
        this.listenTo(this.model, {
            ...
            "element:remove": this.removeElement
        });
    ...
    globals.removeElement = function (element) {
        element.unSelected();
    }
    ...
    contructorElement.unSelected = function () {
        var element = this.getDom(),
            menuElement = element.data("menuElement"),
            ghostElement = element.data("ghostElement");
        menuElement && menuElement.remove();
        ghostElement && ghostElement.remove();
    }
    ...
    function TextElement(data) {
        ...
        this.height = data.height;
        this.width = data.width;
        this.style.textAlign = data.style.textAlign;
        this.html = data.html;
        var innerElement = $("<div>").addClass("inner");
        innerElement.css({
            "width": this.width + "px",
    ...
    contructorElement.setLineHeight = function (height) {
        this.style.lineHeight = height;
    }

    contructorElement.setLetterSpacing = function (spacing) {
    ...
    contructorElement.unSelected = function () {
        Element.prototype.unSelected.call(this);
    }
    ...
    function setEventMenu(type, dataElement, menu) {
        var menuElement = menu,
            ...
            menuFontLineHeight = menuElement.find("#input-font-lineheight"),
            menuFontLetterSpacing = menuElement.find("#input-font-letterspacing"),
        ...
        menuFontLetterSpacing.on("change", setEventChangeLetterSpacing)
        menuFontLineHeight.on("change", setEventChangeLineHeight)
        ...
        menuFontLetterSpacing.val(dataElement.style.letterSpacing);
        menuFontLineHeight.val(dataElement.style.lineHeight);
    ...
    function setEventChangeLineHeight(event) {
        var seft = $(this), menuElement = seft.parents("#text-menu"), dataElement = menuElement.data("dataElement"), domElement = dataElement.getDom();
        var inner = domElement.children(".inner");
        var lineHeight = seft.val(), translateY = (1.4 - lineHeight) * 0.5
        inner.css({"line-height": lineHeight, "transform": "translateY(" + translateY + "em)"});
        domElement.trigger("change");
        dataElement.setLineHeight(lineHeight);
    }

    function setEventChangeLetterSpacing(event) {
        var seft = $(this), menuElement = seft.parents("#text-menu"), dataElement = menuElement.data("dataElement"), domElement = dataElement.getDom();
        var letterSpacing = seft.val();
        letterSpacing = letterSpacing / 1000;
        domElement.css("letter-spacing", letterSpacing + "em");
        domElement.trigger("change");
        dataElement.setLetterSpacing(letterSpacing);
    ...
    function setEventChangeDelete(event) {
        ...
        //domElement.remove();
        dataElement.trigger("element:remove");
    }
